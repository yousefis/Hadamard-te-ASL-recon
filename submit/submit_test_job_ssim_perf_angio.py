...
path_to_model='/exports/lkeb-hpc/syousefi/Code/ASL_LOG/ssim_perf_angio/experiment-11/unet_checkpoints/'
def submit_job():
    ...
    main_script = 'run_ssim_perf_angio.py'
    folder_script = 'functions'
    new_nm=path_to_model + 'results/submit_test_job_ssim_perf_angio.py'
    shutil.copyfile(os.path.realpath(__file__),new_nm)
    # A backup from all files are created. So later if you modify the codes, this does not affect the submitted code.
    backup_script_address, backup_number = backup_script(script_address=new_nm, main_script=main_script, folder_script=folder_script)
    job_name = 'Synth_'+str(backup_number)
    backup_script_address='/exports/lkeb-hpc/syousefi/Code/synthesize_asl_normal_loss/test_network/test_ssim_perf_angio.py'
...
def write_and_submit_job(setting, manager, job_name, script_address):
    ...
    backup_folder = path_to_model.rsplit('/', maxsplit=1)[0]
...
def backup_script(script_address, main_script, folder_script):
    ...
    backup_root_folder = script_folder + 'ASL_LOG/CodeCluster/'
