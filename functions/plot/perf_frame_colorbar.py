...
if __name__=='__main__':
    ...
    for i in range(0, 7):
        ...
        if i == 0:
            h = plt.ylabel('Target', rotation=0, fontsize=20, labelpad=45)
        ...
        if i == 0:
            h= plt.ylabel('MSE', rotation=0, fontsize=20, labelpad=45)
        ...
        if i == 0:
            h=plt.ylabel('SSIM', rotation=0, fontsize=20, labelpad=45)
        im2 = axs[2,7*1+i].imshow(((img_ssim[slice, from_:to_, from_:to_] - img_gt[ slice,from_:to_, from_:to_])),vmin=-error_rng, vmax=error_rng, cmap='jet')
        plt.sca(axs[2, 7*1+i])
        plt.xticks([])
        plt.yticks([])
        ...
        if i == 0:
            h=plt.ylabel('PL', rotation=0, fontsize=20, labelpad=45)
        im2 = axs[3,7*1+i].imshow(((img_perceptual[slice, from_:to_, from_:to_]- img_gt[slice,from_:to_,  from_:to_])), vmin=-error_rng, vmax=error_rng,cmap='jet')
        plt.sca(axs[3, 7 * 1 + i])
        plt.xticks([])
        plt.yticks([])
        ...
        if i == 0:
            h=plt.ylabel('ML-SSIM', rotation=0, fontsize=20, labelpad=45)
        im2 = axs[4, 7 * 1 + i].imshow(((img_multistage[slice,from_:to_,  from_:to_] - img_gt[slice,from_:to_,  from_:to_])),vmin=-error_rng, vmax=error_rng, cmap='jet')
        plt.sca(axs[4, 7 * 1 + i])
        plt.xticks([])
        plt.yticks([])
