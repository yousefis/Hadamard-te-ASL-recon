...
class call_synthesize_net_mse_angio_perf:
    ...
    def run_net(self, loadModel=0):
        ...
        # [m,M]=max_min_data(train_data)
        # [m,M]=max_min_data(validation_data,m,M)
        # [m,M]=max_min_data(test_data,m,M)
        #
        # [m, M] = max_min_data(train_data,1)
        # [m, M] = max_min_data(validation_data, m, M,1)
        # [m, M] = max_min_data(test_data, m, M,1)
        # ======================================
        ...
        with tf.name_scope('Loss'):
            loss_dic = self.loss_instance.loss_selector('MSE_perf_angio',
                                                        labels=labels, logits=logits,
                                                        angio_mse_weight=0.01)
            cost = tf.reduce_mean(loss_dic["loss"], name="cost")
            cost_angio = tf.reduce_mean(loss_dic["angio_mse"], name="angio_mse")
            cost_perf = tf.reduce_mean(loss_dic["perf_mse"], name="perf_mse")
        ...
        with tf.name_scope('MSE_quantifications'):
            [WM_mse, GM_mse] = self.loss_instance.mse_seg(segmentation=segmentation, logits=logits, labels=labels)

        gm_sig_tens = tf.placeholder(tf.float32, name='gm_sig_tens')
        wm_sig_tens = tf.placeholder(tf.float32, name='wm_sig_tens')
        gm_mse_tens = tf.placeholder(tf.float32, name='gm_mse_tens')
        wm_mse_tens = tf.placeholder(tf.float32, name='wm_mse_tens')
        ...
        tf.summary.scalar("mse_Quantification/WM_mse", WM_mse)
        tf.summary.scalar("mse_Quantification/GM_mse", GM_mse)
        ...
        point = 0
        if loadModel:
            chckpnt_dir = '/exports/lkeb-hpc/syousefi/Code/ASL_LOG/mse_perf_angio/experiment-11/unet_checkpoints/'
        ...
        for epoch in range(self.total_epochs):
            while self.no_sample_per_each_itr * int(point / self.no_sample_per_each_itr) < self.sample_no:
                ...
                while (validation_step * self.batch_no_validation < settings.validation_totalimg_patch):
                    ...
                    [loss_vali, out, wm_sig_vl, gm_sig_vl, WM_mse_vl, GM_mse_vl] = sess.run(
                        [cost, y, wm_sig, gm_sig, WM_mse, GM_mse],
                    ...
                    WM_mse_val += WM_mse_vl
                    GM_mse_val += GM_mse_vl
                ...
                loss_validation_p_mse = loss_validation_p_mse / (validation_step)
                loss_validation_p_perceptual = loss_validation_p_perceptual / (validation_step)
                dsc_validation = dsc_validation / (validation_step)

                wm_sig_val = wm_sig_val / (validation_step)
                gm_sig_val = gm_sig_val / (validation_step)
                WM_mse_val = WM_mse_val / (validation_step)
                GM_mse_val = GM_mse_val / (validation_step)
                ...
                [sum_validation, out] = sess.run([summ, y],
                                                 feed_dict={img_row1: crush[:, 0, :, :, :, :],
                                                            img_row2: noncrush[:, 1, :, :, :, :],
                                                            img_row3: crush[:, 2, :, :, :, :],
                                                            img_row4: noncrush[:, 3, :, :, :, :],
                                                            ...
                                                            gm_mse_tens: GM_mse_val,
                                                            wm_mse_tens: WM_mse_val
                                                            })
                ...
                while (step * self.batch_no < self.no_sample_per_each_itr):
                    ...
                    while (train_step < self.display_train_step):
                        ...
                        [loss_train1, optimizing, out, degree1,
                         wm_sig_tr, gm_sig_tr, WM_mse_tr, GM_mse_tr] = sess.run(
                            [cost, optimizer, y, degree, wm_sig, gm_sig, WM_mse, GM_mse],
                            feed_dict={img_row1: crush[:, 0, :, :, :, :],
                                       img_row2: noncrush[:, 1, :, :, :, :],
                                       img_row3: crush[:, 2, :, :, :, :],
                                       img_row4: noncrush[:, 3, :, :, :, :],
                        ...
                        WM_mse_tra += WM_mse_tr
                        GM_mse_tra += GM_mse_tr

                        train_step = train_step + 1
                        print(
                            'point: %d, step*self.batch_no:%f , LR: %.15f, loss_train1:%f,memory_percent: %4s, degree: %d' % (
                                int((point + train_step)),
                                step * self.batch_no, self.learning_rate, loss_train1,
                                str(process.memory_percent()), degree1))

                    average_loss_train1 /= self.display_train_step

                    wm_sig_tra = wm_sig_tra / (self.display_train_step)
                    gm_sig_tra = gm_sig_tra / (self.display_train_step)
                    WM_mse_tra = WM_mse_tra / (self.display_train_step)
                    GM_mse_tra = GM_mse_tra / (self.display_train_step)
                    ...
                    [sum_train] = sess.run([summ],
                                           feed_dict={img_row1: crush[:, 0, :, :, :, :],
                                                      ...
                                                      gm_mse_tens: GM_mse_tra,
                                                      wm_mse_tens: WM_mse_tra
                                                      })
