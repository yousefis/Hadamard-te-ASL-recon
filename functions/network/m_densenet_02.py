...
from functions.layers.conv3d_transpose import conv3d_transpose
# !!

class synth_seg_densenet:
    def __init__(self, class_no=14,trainable=True):
    ...
    def differentiable_mask(self, input,indx,scope):
        seg_angio0 = tf.layers.conv3d(input, filters=2,
                                      kernel_size=3,
                                      padding='same',
                                      strides=(1, 1, 1),
                                      activation=None,
                                      dilation_rate=(1, 1, 1),
                                      name=scope+str(indx))
        mask_angio0 = tf.expand_dims(seg_angio0[:, :, :, :, 0] - seg_angio0[:, :, :, :, 1], axis=-1)
        differentiable_round = tf.maximum(mask_angio0, 0)
        differentiable_round = tf.minimum(10000000 * differentiable_round, 1)
    ...
    def densenet(self, img_row1, img_row2, img_row3, img_row4, img_row5, img_row6, img_row7, img_row8, input_dim, is_training):
        ...
        with tf.variable_scope('conv_transpose2'):
            ...
            deconv2 = tf.layers.conv3d_transpose(level_us2,
                                                 ...
                                                 use_bias=False)
        # with tf.variable_scope('conv_transpose2'):
        #     deconv2 = conv3d_transpose(level_us2,
        #                                          filters=14,
        #                                          kernel_size=3,
        #                                          strides=(2, 2, 2),
        #                                          padding='valid',
        ...
        # == == == == == == == == == == == == == == == == == == == == == ==
        # these are for segmenting the perfusion! so we need to feed level_us3!
        seg_mask_perf0 = self.differentiable_mask(level_us3[:, :, :, :, 0, tf.newaxis], indx=0, scope='perf_seg')
        seg_mask_perf1 = self.differentiable_mask(level_us3[:, :, :, :, 1, tf.newaxis], indx=1, scope='perf_seg')
        seg_mask_perf2 = self.differentiable_mask(level_us3[:, :, :, :, 2, tf.newaxis], indx=2, scope='perf_seg')
        seg_mask_perf3 = self.differentiable_mask(level_us3[:, :, :, :, 3, tf.newaxis], indx=3, scope='perf_seg')
        seg_mask_perf4 = self.differentiable_mask(level_us3[:, :, :, :, 4, tf.newaxis], indx=4, scope='perf_seg')
        seg_mask_perf5 = self.differentiable_mask(level_us3[:, :, :, :, 5, tf.newaxis], indx=5, scope='perf_seg')
        seg_mask_perf6 = self.differentiable_mask(level_us3[:, :, :, :, 6, tf.newaxis], indx=6, scope='perf_seg')

        mask_perf = tf.concat((seg_mask_perf0, seg_mask_perf1), axis=-1)
        mask_perf = tf.concat((mask_perf, seg_mask_perf2), axis=-1)
        mask_perf = tf.concat((mask_perf, seg_mask_perf3), axis=-1)
        mask_perf = tf.concat((mask_perf, seg_mask_perf4), axis=-1)
        mask_perf = tf.concat((mask_perf, seg_mask_perf5), axis=-1)
        mask_perf = tf.concat((mask_perf, seg_mask_perf6), axis=-1)

        seg_perf = tf.multiply(mask_perf, y[:, :, :, :, 0:7])

        # these are for segmenting the angio! so we need to feed level_us3!
        seg_mask_angio0 = self.differentiable_mask(level_us3[:, :, :, :, 7, tf.newaxis], indx=7,scope='angio_seg')
        seg_mask_angio1 = self.differentiable_mask(level_us3[:, :, :, :, 8, tf.newaxis], indx=8,scope='angio_seg')
        seg_mask_angio2 = self.differentiable_mask(level_us3[:, :, :, :, 9, tf.newaxis], indx=9,scope='angio_seg')
        seg_mask_angio3 = self.differentiable_mask(level_us3[:, :, :, :, 10, tf.newaxis], indx=10,scope='angio_seg')
        seg_mask_angio4 = self.differentiable_mask(level_us3[:, :, :, :, 11, tf.newaxis], indx=11,scope='angio_seg')
        seg_mask_angio5 = self.differentiable_mask(level_us3[:, :, :, :, 12, tf.newaxis], indx=12,scope='angio_seg')
        seg_mask_angio6 = self.differentiable_mask(level_us3[:, :, :, :, 13, tf.newaxis], indx=13,scope='angio_seg')

        mask_angio=tf.concat((seg_mask_angio0,seg_mask_angio1),axis=-1)
        mask_angio=tf.concat((mask_angio,seg_mask_angio2),axis=-1)
        mask_angio=tf.concat((mask_angio,seg_mask_angio3),axis=-1)
        mask_angio=tf.concat((mask_angio,seg_mask_angio4),axis=-1)
        mask_angio=tf.concat((mask_angio,seg_mask_angio5),axis=-1)
        mask_angio=tf.concat((mask_angio,seg_mask_angio6),axis=-1)

        seg_angio=tf.multiply(mask_angio, y[:,:,:,:,7:14])
        # == == == == == == == == == == == == == == == == == == == == == ==
        ...
        return  y,mask_angio,seg_angio,mask_perf,seg_perf#,dense_out1,dense_out2,dense_out3,dense_out5,dense_out6
