...
# --------------------------------------------------------------------------------------------------------
class call_synthesize_net_ssim_angio_perf:
    ...
    def run_net(self,loadModel=0):
        ...
        [m,M]=max_min_data(train_data, which=0)
        [m,M]=max_min_data(validation_data,m,M, which=0)
        [m,M]=max_min_data(test_data,m,M, which=0)
        print('min ASL crush : %f, max ASL crush : %f' %(m,M))

        [m, M] = max_min_data(train_data, which=1)
        [m, M] = max_min_data(validation_data, m, M, which=1)
        [m, M] = max_min_data(test_data, m, M, which=1)
        print('min ASL noncrush : %f, max ASL noncrush : %f' % (m, M))

        [m, M] = max_min_data(train_data, which=2)
        [m, M] = max_min_data(validation_data, m, M, which=2)
        [m, M] = max_min_data(test_data, m, M, which=2)
        print('min perfusion : %f, max perfusion : %f' % (m, M))

        [m, M] = max_min_data(train_data, which = 3)
        [m, M] = max_min_data(validation_data, m, M, which = 3)
        [m, M] = max_min_data(test_data, m, M, which = 3)
        print('min angio : %f, max angio : %f' % (m, M))
        # ======================================
        ...
        segmentation = tf.placeholder(tf.float32,
                                  shape=[self.batch_no, self.label_patchs_size, self.label_patchs_size, self.label_patchs_size, 1],name='segments')
        ...
        with tf.name_scope('Loss'):
            loss_dic = self.loss_instance.loss_selector('SSIM_perf_angio',
        ...
        point = 0
        if loadModel:
            chckpnt_dir = '/exports/lkeb-hpc/syousefi/Code/ASL_LOG/ssim_perf_angio/experiment-11/unet_checkpoints/'
        ...
        for epoch in range(self.total_epochs):
            while self.no_sample_per_each_itr * int(point / self.no_sample_per_each_itr) < self.sample_no:
                ...
                while (validation_step * self.batch_no_validation < settings.validation_totalimg_patch):
                    ...
                    [loss_vali,out,wm_sig_vl, gm_sig_vl,WM_ssim_vl, GM_ssim_vl] = sess.run([cost,y,wm_sig, gm_sig,WM_ssim, GM_ssim],
                ...
                WM_ssim_val = WM_ssim_val / (validation_step)
                GM_ssim_val = GM_ssim_val / (validation_step)
                ...
                while (step * self.batch_no < self.no_sample_per_each_itr):
                    ...
                    while (train_step <self.display_train_step):
                        ...
                        [loss_train1, optimizing, out,degree1,
                         wm_sig_tr, gm_sig_tr,WM_ssim_tr, GM_ssim_tr ] = sess.run([cost, optimizer, y,degree,wm_sig, gm_sig,WM_ssim, GM_ssim],
                                      feed_dict={img_row1: crush[:, 0, :, :, :, :],
                                                 img_row2: noncrush[:, 1, :, :, :, :],
                                                 img_row3: crush[:, 2, :, :, :, :],
                                                 img_row4: noncrush[:, 3, :, :, :, :],
                    ...
                    WM_ssim_tra = WM_ssim_tra / (self.display_train_step)
                    GM_ssim_tra = GM_ssim_tra / (self.display_train_step)
