...
class invivo_image_class:
    ...
    def read_image_group_for_test(self,group_name,input_size,final_layer=0):
        ...
        for i in range(len(group_name)):
            img=sitk.ReadImage(''.join(group_name[i]))
            pad_sizex=input_size-img.GetSize()[0]
            pad_sizey=input_size-img.GetSize()[1]
            pad_sizez=input_size-img.GetSize()[2]


            img2 = self.image_padding(img=img,
                  padLowerBound=[int(pad_sizex/2), int(pad_sizey/2), int(pad_sizez/2)],
                  padUpperBound=[int(pad_sizex/2), int(pad_sizey/2), int(pad_sizez/2+1)],
                  constant=0)
