...
from functions.network.densenet_net_mri import _densenet
...
def test_all_nets():
    ...
    with tf.name_scope('Loss'):
        loss_dic = loss_instance.loss_selector('SSIM_perf', labels=labels, logits=logits)
    ...
    copyfile('./test_synthesize_net_mri.py',result_path+'/test_synthesize_net_mri.py')
    ...
    for img_indx in range(len(test_set)):
        crush, noncrush, perf, angio,mri,segmentation,spacing, direction, origin = _image_class.read_image_for_test(test_set=test_set, img_indx=img_indx,input_size=in_dim,final_layer=final_layer)
