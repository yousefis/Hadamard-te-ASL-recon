...
from functions.image_reader.invivo_read_data import _invivo_read_data
from functions.image_reader.invivo_image_class import invivo_image_class
...
def test_all_nets():
    ...
    _rd = _invivo_read_data(data=data, reverse=False,
    ...
    for img_indx in range(len(test_data)):
        crush, noncrush, perf, angio, mri, segmentation_, spacing, direction, origin = _image_class.read_image_for_test(
            test_set=test_data, img_indx=img_indx, input_size=in_dim, final_layer=final_layer,invivo=1)
        for i in range(8):
        ...
        [out] = sess.run([y],
                         feed_dict={img_row1: np.expand_dims(np.expand_dims(crush[0], 0), -1),
                                    ...
                                    mri_ph: np.expand_dims(np.expand_dims(crush[0], 0), -1),#useless input
                                    segmentation: np.expand_dims(np.expand_dims(crush[0], 0), -1),#useless input
        ...
        for i in range(np.shape(out)[-1]):
            ...
            sitk.WriteImage(sitk_image, parent_path + result_dir + res_dir + '/' + nm + '_' + str(i % 7) + '.nii')
            print(parent_path + result_dir + res_dir + '/' + nm + '_' + str(i % 7) + '.nii done!')
        for i in range(7):
            if i == 0:
            ...
            sitk.WriteImage(sitk_angio, parent_path + result_dir + res_dir + '/GT/angio_' + str(i) + '.nii')
            ...
            sitk.WriteImage(sitk_perf, parent_path + result_dir + res_dir + '/GT/perf_' + str(i) + '.nii')
