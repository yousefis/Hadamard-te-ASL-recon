...
class _forked_densenet2:
    ...
    def convolution_stack(self, input, stack_name, filters, kernel_size, padding, is_training):
        with tf.variable_scope(stack_name):
            conv1 = self.layers.conv3d(input, filters=filters, kernel_size=kernel_size, padding=padding,
                                       dilation_rate=1, is_training=is_training, trainable=True, scope=stack_name + '2',
                                       reuse=False)
            #
    ...
    def level_design(self,input,level_name,filters1,filters2,is_training,kernel_size,in_size,crop_size,padding1,padding2,flag=2,paddingfree_scope='',filters3=0):
        with tf.variable_scope(level_name):
            conv1=self.layers.conv3d( input, filters=filters1, kernel_size=kernel_size, padding=padding1,
                                dilation_rate=1, is_training=is_training, trainable=True, scope=level_name+'1', reuse=False)
            # conv1 = tf.layers.conv3d(input,
            #                          filters=filters1,
            #                          kernel_size=kernel_size,
            #                          padding=padding1,
            ...
            # conv3 = bn
            if flag==1:
                conc = self.paddingfree_conv(input=conc, filters=filters3, kernel_size=3, is_training=is_training,scope=paddingfree_scope)
                crop = conc[:,
    ...
    def paddingfree_conv(self,input,filters,kernel_size,is_training,scope):
        conv = self.layers.conv3d(input, filters=filters, kernel_size=kernel_size, padding='valid',
                                   dilation_rate=1, is_training=is_training, trainable=True, scope= scope,
                                   reuse=False)
    ...
    def perfusion_angio(self,input,crop3,crop2,crop1,is_training,in_size0,crop_size0,scope_name):
        # == == == == == == == == == == == == == == == == == == == == == ==
        # perf


        with tf.variable_scope('conv_trans1_'+scope_name):
            # deconv1=self.upsampling3d.upsampling3d(level_us1, 'scope1')
            deconv1 = tf.layers.conv3d_transpose(input,
                                                 filters=45,
                                                 kernel_size=3,
                                                 strides=(2, 2, 2),
                                                 padding='valid',
                                                 use_bias=False)
        with tf.variable_scope('concat1'+scope_name):
            conc12 = tf.concat([crop3, deconv1], 4)

        # level 2 of unet

        [level_us2, crop0] = self.level_design(conc12,
                                               'level_us2'+scope_name,
                                               filters1=40,
                                               filters2=35,
        ...
        loss_fm1=self.layers.conv3d(level_us2,
                                    filters=7,
                                    kernel_size=3,
                                    padding='same',
                                    dilation_rate=1,
                                    is_training=is_training,
                                    trainable=True,
                                    scope='lossFM1'+scope_name,
                                    reuse=False)


        with tf.variable_scope('conv_trans2'+scope_name):
            ...
            deconv2 = tf.layers.conv3d_transpose(level_us2,
                                                 ...
                                                 padding='valid',
                                                 use_bias=False)
        with tf.variable_scope('concat2'+scope_name):
            conc23 = tf.concat([crop2, deconv2], 4)

        # level 1 of unet
        [level_us3, crop0] = self.level_design(conc23,
                                               'level_us3'+scope_name,
                                               filters1=25,
                                               filters2=20,
                                               is_training=is_training,
                                               kernel_size=1,
                                               ...
                                               crop_size=crop_size0,
                                               padding1='same', padding2='same')
        loss_fm2 = self.layers.conv3d(level_us3,
                                      filters=7,
                                      kernel_size=3,
                                      padding='same',
                                      dilation_rate=1,
                                      is_training=is_training,
                                      trainable=True,
                                      scope='lossFM2' + scope_name,
                                      reuse=False)

        with tf.variable_scope('conv_transpose3'+scope_name):
            ...
            deconv3 = tf.layers.conv3d_transpose(level_us3,
                                                 ...
                                                 padding='valid',
                                                 use_bias=False)
        with tf.variable_scope('concat3'+scope_name):
            conc34 = tf.concat([crop1, deconv3], 4)

        # level 1 of unet
        [level_us4, crop0] = self.level_design(conc34,
                                               'level_us4'+scope_name,
                                               filters1=14,
                                               filters2=7,
                                               is_training=is_training,
                                               kernel_size=1,
        ...
        y = self.layers.conv3d(level_us4, filters=7, kernel_size=3, padding='same',
                                   dilation_rate=1, is_training=is_training, trainable=True, scope='last_layer'+scope_name,
                                   reuse=False)

            # conv1 = tf.layers.conv3d(level_us4,
            #                          filters=7,
            #                          kernel_size=3,
            #                          padding='same',
            #                          activation=None,
    ...
    def densenet(self, img_row1, img_row2, img_row3, img_row4, img_row5, img_row6, img_row7, img_row8, input_dim, is_training):
        ...
        in_size3 = tf.to_int32((in_size2 - 4))  # level_design1
        ...
        # in_size3 = ((in_size2-4))  # level_design1
        ...
        # == == == == == == == == == == == == == == == == == == == == == ==
        with tf.variable_scope('perfusion'):
            perfusion,perf_loss_fm1,perf_loss_fm2= self.perfusion_angio( input=level_us1[:,:,:,:,0:35], crop3=crop3, crop2=crop2, crop1=crop1,
                                             is_training=is_training, in_size0=in_size0, crop_size0=crop_size0,
                                             scope_name='perfusion')
        with tf.variable_scope('angio'):
            angio,angio_loss_fm1,angio_loss_fm2 = self.perfusion_angio(input=level_us1[:, :, :, :, 35:-1], crop3=crop3, crop2=crop2, crop1=crop1,
                                             is_training=is_training, in_size0=in_size0, crop_size0=crop_size0,
                                             scope_name='angio')
        ...
        return  perfusion,perf_loss_fm1,perf_loss_fm2,\
                angio,angio_loss_fm1,angio_loss_fm2
