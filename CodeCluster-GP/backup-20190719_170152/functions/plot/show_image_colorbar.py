...
if __name__=='__main__':
    ...
    for i in range(0, 7):
        ...
        if i == 0:
            plt.ylabel('SSIM')
        im2 = axs[1, 7 * 1 + i].imshow(((img_ssim[slice, from_:to_, from_:to_] - img_gt[slice, from_:to_, from_:to_])),
                                       vmin=vmin, vmax=max, cmap='jet')
        plt.sca(axs[1, 7 * 1 + i])
        plt.xticks([])
        plt.yticks([])
        ...
        if i == 0:
            plt.ylabel('PL')
        im2 = axs[2, 7 * 1 + i].imshow(
            ((img_perceptual[slice, from_:to_, from_:to_] - img_gt[slice, from_:to_, from_:to_])), vmin=vmin, vmax=max,
            cmap='jet')
        plt.sca(axs[2, 7 * 1 + i])
        plt.xticks([])
        plt.yticks([])
        ...
        im2 = axs[3, 7 * 1 + i].imshow(
            ((img_multistage[slice, from_:to_, from_:to_] - img_gt[slice, from_:to_, from_:to_])), vmin=vmin, vmax=max,
            cmap='jet')
        plt.sca(axs[3, 7 * 1 + i])
        plt.xticks([])
        plt.yticks([])
        ...
        if i == 0:
            plt.ylabel('GT')
        im2 = axs[4, 7 * 1 + i].imshow(((img_gt[slice, from_:to_, from_:to_] - img_gt[slice, from_:to_, from_:to_])),
                                       vmin=vmin, vmax=max, cmap='jet')
        plt.sca(axs[4, 7 * 1 + i])
        plt.xticks([])
        plt.yticks([])
