...
class _read_data:
    def __init__(self, data,reverse, train_tag='', validation_tag='',
        ...
        if Server == 'DL':
            ...
            self.atlas_path = '/srv/2-lkeb-17-dl01/syousefi/TestCode/EsophagusProject/Data-01/atlas/Brainweb/'

        else:
            path = '/exports/lkeb-hpc/syousefi/Synth_Data/'
            self.atlas_path = '/exports/lkeb-hpc/syousefi/Data/Original_altas/BrainWeb/'
    ...
    def read_data_nonsmooth(self, data_dir):
        ...
        for pd in data_dir:
            ...
            crush = [join(image_path, pd, dt) for dt in listdir(join(image_path, pd)) if
                    isfile(join(image_path, pd, dt))and dt.startswith('tissue_sig')]
            crush.sort()#reverse=self.reverse)
            non_crush = [join(image_path, pd, dt) for dt in listdir(join(image_path, pd)) if
                    isfile(join(image_path, pd, dt))and dt.startswith('sig_')and not dt.startswith('sig__smooth')]
    ...
    def read_data_path(self):  # join(self.resampled_path, f)
        ...
        all_len=len(data_dir)

        train_indx1 = list(range(0, int(all_len*.7)))
        valid_indx1 = list(range(int(all_len*.7)+1, int(all_len*.8)))
        test_indx1 = list(range(int(all_len*.8)+1, all_len))

        train_data = self.read_data_nonsmooth(
            np.hstack((data_dir[train_indx1])))
        validation_data = self.read_data_nonsmooth(
            np.hstack((data_dir[valid_indx1])))
        test_data = self.read_data_nonsmooth(
            np.hstack((data_dir[test_indx1])))
        print(
            'train: %d dataset#1 \nvalidation: %d dataset#1 \ntest: %d dataset#1 \ntotal: %d' %
            (len(data_dir[train_indx1]),  len(data_dir[valid_indx1]),
