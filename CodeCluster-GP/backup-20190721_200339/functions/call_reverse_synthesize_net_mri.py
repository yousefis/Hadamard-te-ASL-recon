...
from functions.network.reverse_densenet import _reverse_densenet
...
# --------------------------------------------------------------------------------------------------------
class call_reverse_synthesize_net_mri:
    def __init__(self, data, sample_no, validation_samples, no_sample_per_each_itr,
        ...
        self.label_patchs_size =63#39
        self.patch_window =77 #53#89
        self.sample_no = sample_no
        self.batch_no = 8
        self.batch_no_validation = 8
    ...
    def run_net(self,loadModel = 0):
        ...
        # ======================================
        # tissue=1
        # [m, M] = max_min_data(train_data,tissue=tissue)
        # [m, M] = max_min_data(validation_data, m, M,tissue=tissue)
        # [m, M] = max_min_data(test_data, m, M,tissue=tissue)
        # print(M,m,M/m)
        #
        # tissue=0
        # [m, M] = max_min_data(train_data, tissue=tissue)
        ...
        # unet=_unet()
        reverse_densenet = _reverse_densenet()

        y,degree = reverse_densenet.reverse_densenet(img_row1=img_row1,
                              img_row2=img_row2,
                              img_row3=img_row3,
                              img_row4=img_row4,
                              img_row5=img_row5,
        ...
        for epoch in range(self.total_epochs):
            while self.no_sample_per_each_itr * int(point / self.no_sample_per_each_itr) < self.sample_no:
                ...
                while (validation_step * self.batch_no_validation < settings.validation_totalimg_patch):
                    ...
                    [loss_vali,wm_sig_vl, gm_sig_vl,WM_ssim_vl, GM_ssim_vl] = sess.run([cost,wm_sig, gm_sig,WM_ssim, GM_ssim],
                ...
                while (step * self.batch_no < self.no_sample_per_each_itr):
                    ...
                    while (train_step <self.display_train_step):
                        ...
                        [loss_train1, optimizing, out,degree1 ,
                         wm_sig_tr, gm_sig_tr, WM_ssim_tr, GM_ssim_tr] = sess.run([cost, optimizer, y,degree,
