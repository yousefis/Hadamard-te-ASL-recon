...
class _forked_densenet:
    ...
    def densenet(self, img_row1, img_row2, img_row3, img_row4, img_row5, img_row6, img_row7, img_row8, input_dim, is_training):
        ...
        deconv1 = conv3d_transpose(level_ds3, filters=28, kernel_size=3, padding='valid', bn_training=1, strides=(2, 2, 2),
                 scope='conv_transpose1', activation=None, use_bias=False, initializer='trilinear', trainable=True)
        ...
        deconv2 = conv3d_transpose(level_us2, filters=14, kernel_size=3, padding='valid', bn_training=1, strides=(2, 2, 2),
                         scope='conv_transpose2', activation=None, use_bias=False, initializer='trilinear',
                         trainable=True)
        ...
        seg_angio0 = tf.reduce_max(tf.layers.conv3d(y[:,:,:,:,0,tf.newaxis], filters=2, kernel_size=1, padding='same', strides=(1, 1, 1),
                         activation=None, dilation_rate=(1, 1, 1), name='angio_seg0'),axis=-1)[...,tf.newaxis]
        seg_angio1 = tf.reduce_max(tf.layers.conv3d(y[:, :, :, :, 0, tf.newaxis], filters=2, kernel_size=1, padding='same',
                                      strides=(1, 1, 1),
                                      activation=None, dilation_rate=(1, 1, 1), name='angio_seg1'),axis=-1)[...,tf.newaxis]
        seg_angio2 = tf.reduce_max(tf.layers.conv3d(y[:, :, :, :, 0, tf.newaxis], filters=2, kernel_size=1, padding='same',
                                      strides=(1, 1, 1),
                                      activation=None, dilation_rate=(1, 1, 1), name='angio_seg2'),axis=-1)[...,tf.newaxis]
        seg_angio3 = tf.reduce_max(tf.layers.conv3d(y[:, :, :, :, 0, tf.newaxis], filters=2, kernel_size=1, padding='same',
                                      strides=(1, 1, 1),
                                      activation=None, dilation_rate=(1, 1, 1), name='angio_seg3'),axis=-1)[...,tf.newaxis]
        seg_angio4 = tf.reduce_max(tf.layers.conv3d(y[:, :, :, :, 0, tf.newaxis], filters=2, kernel_size=1, padding='same',
                                      strides=(1, 1, 1),
                                      activation=None, dilation_rate=(1, 1, 1), name='angio_seg4'),axis=-1)[...,tf.newaxis]
        seg_angio5 = tf.reduce_max(tf.layers.conv3d(y[:, :, :, :, 0, tf.newaxis], filters=2, kernel_size=1, padding='same',
                                      strides=(1, 1, 1),
                                      activation=None, dilation_rate=(1, 1, 1), name='angio_seg5'),axis=-1)[...,tf.newaxis]
        seg_angio6 = tf.reduce_max(tf.layers.conv3d(y[:, :, :, :, 0, tf.newaxis], filters=2, kernel_size=1, padding='same',
                                      strides=(1, 1, 1),
                                      activation=None, dilation_rate=(1, 1, 1), name='angio_seg6'),axis=-1)[...,tf.newaxis]

        seg_angio = (y[7:14])

        synth_angio=tf.multiply(seg_angio, y[7:14])
        ...
        return  synth_perf,synth_angio,seg_angio#,dense_out1,dense_out2,dense_out3,dense_out5,dense_out6
